...
public class SQLiteTasks extends Application {
    private static final Logger logger = Logger.getLogger(SQLiteTasks.class.getName());

    private static final String[] SAMPLE_NAME_DATA = { "Гвасалия Р.Г.", "Шарфи Ю.Н.", "Геркулов Д.А.", "Жарова М.В." };
    ...
    @Override public void start(Stage stage) throws InterruptedException, ExecutionException {
        ...
        final ListView<String> sectionColorCombo = new ListView<>();
        final ListView<String> embryologistView = new ListView<>();
        final ListView<String> doctorView = new ListView<>();
        ...
        clearNameList.setOnAction(event -> doctorView.getItems().clear());
...
private void fetchNamesFromDatabaseToListView(
        ...
        @Override protected ObservableList<String> call() throws Exception {
         ...
         //   Thread.sleep(1000);
            ...
            try (Connection con = getConnection()) {
                return fetchDoctorNames(con);
            }
        }

        private ObservableList<String> fetchDoctorNames(Connection con) throws SQLException {
            logger.info("Fetching names from database");
            ObservableList<String> names = FXCollections.observableArrayList();

            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("select control_value from doctors");
            while (rs.next()) {
                names.add(rs.getString("control_value"));
            }

            logger.info("control_value " + names.size() + " names");
    ...
    class DBSetupTask extends DBTask {
        ...
        private boolean schemaExists(Connection con) {
            ...
            try {
                Statement st = con.createStatement();
                st.executeQuery("select count(*) from doctors");
